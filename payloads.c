...
)
{
	...
	{
		int tyy = t % (nSamplesPerSec / 400);
		INT nFreq = (INT)(FastSine((FLOAT)tyy / 10.f) * 100.f + 500.f);
		FLOAT fSine = FastSine((FLOAT)tyy / 10.f) * (FLOAT)nSamplesPerSec;
		psSamples[t] = (SHORT)(TriangleWave(t, nFreq, (FLOAT)nSamplesPerSec * 5.f + fSine) * (FLOAT)SHRT_MAX * .1f) +
			(SHORT)(SquareWave(t, nFreq, nSampleCount) * (FLOAT)SHRT_MAX * .2f + (tyy * (t >> 11) >> 8));
	}
}
...
)
{
	...
	{
		BYTE bFreq = (BYTE)((t | t % 255 | t % 257) + (t & t >> 8) + (t * t * t * max(1, 42 & t >> 10)) + ((t % ((t >> 8 | t >> 16) + 1)) ^ t));
...
)
{
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			...
			{
				INT t = z * nCubeRoot * nCubeRoot + y * nCubeRoot + x;
				if (t >= nSampleCount)
					continue;

				INT nFreq = (INT)((FLOAT)(y & z & x) * FastSine((FLOAT)(z * y * x) / 100.f));
				psSamples[t] = (byte)(nFreq * t + t * 16 * (int)(TriangleWave(t >> 12, 1, 8) * 8) % 250 + t % 250) * 0xff;
			}
		}
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / (2 + t / (nSampleCount / 1000))) * 100.f + 500.f);
		psSamples[t] = (SHORT)(nFreq * (FLOAT)SHRT_MAX * .1f);
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		SHORT sFreq = (byte)((t * t * (t >> 12 & t) >> (t >> 12 & t)));
		psSamples[t] = sFreq * 128;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		SHORT bFreq = (BYTE)((t | t >> 9 | t >> 7 | t << 4 | t >> t) * t & (t >> 9 | t >> 11));
		psSamples[t] = bFreq * 128;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		SHORT bFreq = (BYTE)(t * (t >> 12 & (t >> 2 | t >> 8) & 30) * 4 - t * 2);
		(psSamples)[t] = bFreq * 128;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		...
		if (tr) {
			psSamples[t] = (SHORT)(((int)(SawtoothWave(t, sRand, nSampleCount) * 256) ^ (int)(SawtoothWave(t, 1, nSampleCount) * 256)))
				& ~sRand | ((SHORT)XorYeet32() >> 12) +
				(SHORT)(SineWave(XorYeet32() % nSampleCount, nRand ^ sRand, nSampleCount) * (FLOAT)SHRT_MAX * .1f) +
				(SHORT)(TriangleWave(t / 16, 3000, nSampleCount) * (FLOAT)SHRT_MAX);
		}
		else
		{
			psSamples[t] = (byte)(t * 256 * sRand / nSampleCount ^ t) * 255;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y | x) * FastSine((FLOAT)(y * x) / 500.f));
			psSamples[nIndex] =
				(SHORT)(SquareWave(y & x, (nFreq % 500) + 1000, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) ^ (SHORT)(nIndex % (nSamplesPerSec / 100) * 256);
		}
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		BYTE bFreq = (BYTE)((t * (t >> 7 | t >> 2 | ((int)(t * 1.0078125) & t)) + t / 2));
		(psSamples)[t] = bFreq * 128;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		BYTE bFreq = (BYTE)(t * (t >> 9 | 9) % 256 / 2 - t * (t >> 9 | 10) % 256 / 2 + t * (t >> 9 | 9) ^ t);
		(psSamples)[t] = bFreq * 255;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(TriangleWave(__rdtsc() % 8, 1500, nSampleCount) * (FLOAT)SHRT_MAX * .3f) |
			(SHORT)(SquareWave(__rdtsc() % 8, 1000, nSampleCount) * (FLOAT)SHRT_MAX * .3f) + (SHORT)~t + ((SHORT)t >> 2) + (SHORT)((BYTE)(t * (t >> 9 & t >> 10 ^ t) + t) * 0xff);
	}
}
...
{
	UNREFERENCED_PARAMETER(nSamplesPerSec);
	int xy = XorYeet32() % 0x1000;
	int d = nSampleCount;
	for (INT t = 0; t < nSampleCount; t++)
	{
		d = max(nSampleCount - t, 2400) / 100;
		if (!(XorYeet32() % d)) {
			xy = XorYeet32() % 0x1000;
		}
		psSamples[t] = ((SHORT)(SawtoothWave(__rdtsc() % 1500, 1500, nSampleCount) * (FLOAT)SHRT_MAX * .3f) ^
			((SHORT)(SawtoothWave(t, t % 1000, nSampleCount) * (FLOAT)SHRT_MAX * .1f) >> 8)) + (SHORT)((BYTE)(t * xy >> 12) * 0xff);
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] = (SHORT)(SineWave(x + y, (nFreq % 1000) + 1000, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f + (BYTE)((x & y) / 5 * 4) * 255 * .3f);
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y - x) * FastCosine((FLOAT)(y | x) / 10.f));
			psSamples[nIndex] = (SHORT)(SineWave(y, (nFreq % 100) + 100, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
	...
	INT nSquareRoot = (INT)sqrtf((FLOAT)nSampleCount) + 1;
		for (INT y = 0; y < nSquareRoot; y++)
		{
			for (INT x = 0; x < nSquareRoot; x++)
			{
				...
				psSamples[nIndex] = (SHORT)(SineWave(y - (x % (y + 1)), (nFreq % 100) + 50, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
			}
		}
}
VOID
...
)
{
	...
	for (int t = 0; t < nSampleCount; ++t) {
		byte bytebeat = (byte)(t * (((t * 127 / 128 ^ t * 129 / 128) >> 7 & 1) + ((t * 255 / 256 ^ t * 257 / 256) >> 7 & 1)) * 64 % 256 * (1 - fmod(t / 262144. , 1)) * (~t >> 17 & 1) + (t * (t >> 7 | t * 97)) % 256 * min(1, fmod(t / 131072., 2)));
		psSamples[t] = bytebeat * 255;
	}
}
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				((BYTE*)psSamples)[nIndex] = (BYTE)((((x & y & z) * x * y * z >> 8) & 255) / 2 + ((nIndex * (nIndex & nIndex >> 9) >> 15 ^ nIndex >> 1) & 255) / 2);
			}
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst.r += (224^prgbSrc[y * w + x].r) / 64;
			rgbDst.g += (224^prgbSrc[y * w + x].g) / 64;
			rgbDst.b += (224^prgbSrc[y * w + x].b) / 64;
...
{
	int ys = XorYeet32() % h;
	int size = XorYeet32() % 80 + 50;

	if (XorYeet32() % 2 + 1 == 1) {
		for (int i = 0; i < 5; i++) {
			BitBlt(hdcDesktop, i + 1, ys, w, size, hdcDesktop, 0, ys, SRCINVERT);
		}
	}
	else {
		for (int i = 0; i < 5; i++) {
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			DWORD rgb = prgbSrc[y * w + x].rgb + ((0x101010 | (t & y) ^ ((t & x) << 8) ^ (t << 16)) + 1) + XorYeet32() % 142;
			if (!rgb)
			{
				rgb = 2;
			}
			rgbDst = prgbDst[y * w + x];
			rgbDst.rgb += rgb;
			if (!rgbDst.rgb)
			{
				rgbDst.rgb = 0xFFFFFF;
...
{
	BitBlt(hdcDesktop, XorYeet32() % 50 - 25, XorYeet32() % 50 - 25, w, h, hdcDesktop, 0, 0, SRCINVERT);
	if (!(t % 16))
	{
		RedrawWindow(NULL, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_ALLCHILDREN);
	}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			FLOAT a = FastCosine(_y + _t) * 16.f; FLOAT beee = FastSine(_y + _t) * 16.f;

			u = x + (INT)a, v = y + (INT)beee;
			u %= w;
			v %= h;
			...
			prgbDst[y * w + x].rgb ^= HSLToRGB(hsl).rgb;
			prgbDst[y * w + x].rgb = min((u + t ^ v + t) * chosen, (DWORD)(prgbDst[y * w + x].rgb - (u + t ^ v + t) * chosen));
		}
	}
}
...
)
{
	...
	for (INT i = 64; i > 8; i -= 8)
	{
		HBRUSH hbrBall;
		HPEN hpenBall;
		hbrBall = CreateSolidBrush(nShaderThreeSeed * i + t * 32);
		hpenBall = CreatePen(PS_SOLID, 2, 0xFFFFFF * (XorYeet32() & 1));
		...
		hbrBall = GetStockObject(NULL_BRUSH);
		SelectObject(hdcDst, hbrBall);
		Ellipse(hdcDst, x + rcBounds.left - i + 64, y + rcBounds.top - i + 64, x + rcBounds.left + i - 64, y + rcBounds.top + i - 64);
...
)
{
	...
	BitBlt(hdcDesktop, FastCosine(t) * 16, FastSine(t) * 16, w, h, hdcDesktop, 0, 0, SRCCOPY);
	BitBlt(hdcDst, rcBounds.left, rcBounds.top, w, h, hdcTemp, 0, 0, SRCCOPY);
...
)
{
	...
	BOOL bShiftDir = (BOOL)(XorYeet32() & 1);
	BYTE bChannels = (BYTE)(XorYeet32() & 0b111);
	RGBQUAD rgbSrc;
	RGBQUAD rgbDst;
...
)
{
	...
	RGBQUAD rgbDst;
	RGBQUAD rgbSrc;
	BOOL bOperation = (BOOL)(XorYeet32() % 3);
...
)
{
	...
	for (INT i = 0; i < 5; i++)
	{
		INT nBlockSize = XorYeet32() % 129 + 128;
		INT nNewBlockSize = nBlockSize + (XorYeet32() % 17 + 16);
		INT x = XorYeet32() % (w - nBlockSize);
		INT y = XorYeet32() % (h - nBlockSize);
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			if ((roundf(hsl.h * 10.f) / 10.f) != (roundf((FLOAT)((XorYeet32() + t) % 257) / 256.f * 10.f) / 10.f))
			{
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = SquareWave(t + y, 60, y) * 10.f;
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb = ((rgbDst.b - 1 & 0xff) | ((rgbDst.g + 1 & 0xff) << 8) | ((rgbDst.r - 2 & 0xff) << 16));
			prgbDst[y * w + x].rgb ^= rgbDst.rgb;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 4 + y, 10, h + y) * 10.f;
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb = ((rgbDst.b + 1000) + ((rgbDst.g + 1000) << 8) + ((rgbDst.r + 1000) << 16)) & 0xffffff;
			if (XorYeet32() % 20 == 0) {
				rgbDst.rgb = ~rgbDst.rgb;
			}
			prgbDst[y * w + x] = rgbDst;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 4 + y, .1, h) * SquareWave(t * 2 + y, .05, h) * 10.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			FLOAT a = cosf(atan2f((FLOAT)((y * 32) & t), (FLOAT)((x * 32) & t))) * logf((FLOAT)(t | 64)) * 32.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = HSLToRGB(hsl);
			prgbDst[y * w + x].rgb ^= rgbDst.rgb | 1;
			prgbDst[y * w + x].rgb *= 0xff;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 8 + y / 2, 32, w) * SineWave(t * 8 + y / 2, 16, h) * 4.f;

		for (INT x = 0; x < w; x++)
		{
			FLOAT b = TriangleWave(t * 8 + x / 2, 32, w) * SineWave(t * 8 + y / 2, 16, h) * 4.f;
			...
			rgbDst.rgb = HSLToRGB(hsl).rgb * 0xff;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb -= (COLORREF)(__rdtsc() & 0x101010 & (t + __rdtsc() & 0x101010));
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb = rgbDst.rgb ^ crRainbow;
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	{
		rgbDst.rgb = (XorYeet32() % 256) * 0x010101;
